...
const NewsCard = ({ data }) => {
  return (
    <div className="max-w-4xl mx-auto my-10 bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden font-sans">
      <div className="flex items-center justify-between p-4 bg-gray-200 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <h4 className="text-xl font-semibold text-gray-950">{data.author.name}</h4>
            <p className="text-gray-600">{data.author.published_date}</p>
          </div>
        </div>
      ...
      <div className="p-4 md:p-6 lg:p-8 space-y-6">
        ...
        <div className="space-y-3">
          ...
          <Link href={`/news/${data._id}`} className="inline-block text-[#ED6F33] font-semibold text-xl hover:underline">
            Read More
          </Link>
        </div>
        <div className="pt-6 border-t border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xl font-bold text-gray-900">{data.rating.number}</span>
            </div>
            <div className="flex items-center gap-2.5 text-gray-600">
              <FaRegEye className="w-7 h-7" />
              <span className="text-xl font-semibold">{data.total_view}</span>
            </div>
          </div>
        </div>
      </div>
